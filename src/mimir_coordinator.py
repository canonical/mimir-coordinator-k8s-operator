...
class MimirCoordinator:
    ...
    def is_coherent(self):
        ...
        roles = self.roles()
        return set(roles).issubset(MINIMAL_DEPLOYMENT)
    ...
    def roles(self) -> Counter[str]:
        ...
        for relation in self.relations:
            ...
            try:
                ...
                logger.error(f'Could not load roles from relation {relation!r}',
                             exc_info=True)
                continue
    ...
    def _relation_data_valid(self, relation: Relation, unit: Optional[Unit] = None) -> bool:
        ...
        for unit in units_to_check:
            try:
                schema().validate(
                    dict(app=relation.data[relation.app],
                         unit=relation.data[unit]))
            except (pydantic.ValidationError, ModelError, KeyError):
                logger.error(f'relation data invalid: {relation.data}',
                             exc_info=True)
                return False
        return True
