...
from mimir_coordinator import MimirCoordinator, MimirRole, MINIMAL_DEPLOYMENT, RECOMMENDED_DEPLOYMENT
...
def test_coherent(roles, expected):
    relations = [
                    Relation(
                        endpoint=_to_endpoint_name(role),
                        remote_unit_ids=list(range(n_units))
    ...
    def pre_event(charm: MimirCoordinatorK8SOperatorCharm):
        charm_relations = list(chain(
            *(charm.model.relations.get(_ep, []) for _ep in ALL_MIMIR_RELATION_NAMES)
        ))
        mc = MimirCoordinator(charm_relations)
        assert mc.is_coherent() is expected

    with patch("mimir_coordinator.MimirCoordinator._relation_data_valid",
               lambda *_: True):
