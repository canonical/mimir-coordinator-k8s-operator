...
def coordinator():
    ...
    coord.cluster.gather_addresses_by_role = MagicMock(return_value={
    ...
    coord.cluster.gather_addresses = MagicMock(return_value=["http://some.mimir.worker.0:8080", "http://some.mimir.worker.1:8080"])
...
def topology():
    top = MagicMock()
    top.as_dict = MagicMock(return_value={
        "model": "some-model",
...
@pytest.mark.parametrize("addresses_by_role, replication", [
...
def test_build_memberlist_config(mimir_config, topology, coordinator):
    ...
    expected_config = {"cluster_label": "some-model_some-uuid_mimir", "join_members": ["http://some.mimir.worker.0:8080", "http://some.mimir.worker.1:8080"]}
